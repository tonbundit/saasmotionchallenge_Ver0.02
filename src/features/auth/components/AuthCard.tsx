"use client";

import { Card } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { motion } from "framer-motion";

export function AuthCard() {
  return (
    <motion.div
      initial={{ opacity: 0, scale: 0.9, y: 20 }}
      animate={{ opacity: 1, scale: 1, y: 0 }}
      transition={{ type: "spring", damping: 15 }}
    >
      <Card className="max-w-md w-full p-10 bg-[#FFFFFF]">
        <div className="space-y-6">
          <div className="space-y-2">
            <h1 className="text-4xl font-black uppercase italic tracking-tighter">
              Login to{" "}
              <span className="text-accent underline decoration-4 underline-offset-4">
                Studio
              </span>
            </h1>
            <p className="text-zinc-600 font-bold tracking-tight">
              Enter your credentials to manage your assets.
            </p>
          </div>

          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-black uppercase tracking-widest text-zinc-400">
                Email Address
              </label>
              <Input type="email" placeholder="senior@architect.dev" />
            </div>

            <div className="space-y-2">
              <label className="text-sm font-black uppercase tracking-widest text-zinc-400">
                Password
              </label>
              <Input type="password" placeholder="••••••••" />
            </div>
          </div>

          <div className="pt-4 flex flex-col gap-4">
            <Button size="lg" className="w-full" variant="accent">
              Let&apos;s Go!
            </Button>

            <div className="flex items-center gap-2">
              <div className="h-[2px] flex-1 bg-black" />
              <span className="text-xs font-black uppercase">
                Or Connect with
              </span>
              <div className="h-[2px] flex-1 bg-black" />
            </div>

            <Button variant="outline" className="w-full font-black">
              <svg
                viewBox="0 0 24 24"
                className="w-5 h-5 mr-2"
                fill="currentColor"
              >
                <path
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                  fill="#4285F4"
                />
                <path
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                  fill="#34A853"
                />
                <path
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.25.81-.59z"
                  fill="#FBBC04"
                />
                <path
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                  fill="#EA4335"
                />
              </svg>
              Google Account
            </Button>
          </div>

          <p className="text-center text-xs font-black uppercase text-zinc-400">
            Don&apos;t have an account?{" "}
            <a href="#" className="underline text-black">
              Sign Up Free
            </a>
          </p>
        </div>
      </Card>
    </motion.div>
  );
}
